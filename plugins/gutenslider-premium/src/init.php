...
if (! function_exists('eedee_gutenslider_block_init')) {
    function eedee_gutenslider_block_init() {
        ...
        if (gutenslider_fs()->is__premium_only() ) {
            ...
            $index_js     = 'build/gutenslider-pro.js';
            $script_asset = include $script_asset_path;

            wp_register_script(
                'eedee-gutenslider-block-editor-pro',
                plugins_url($index_js, dirname(__FILE__)),
                $script_asset['dependencies'],
                $script_asset['version']
            );

            $editor_css = 'build/gutenslider-pro.css';
            wp_register_style(
            ...
            wp_localize_script('eedee-gutenslider-block-editor-pro', 'eedeeGutenslider', $eedee_gutenslider_block_variables);
        } else {
        ...
        if (gutenslider_fs()->is__premium_only() ) {
            $editor_script = 'eedee-gutenslider-block-editor-pro';
        } else {
...
add_filter( 'plugin_row_meta', 'eedee_plugin_row_meta', 10, 2 );
...
if (!function_exists('gutenslider_register_block_pattern_categories')) {
	function gutenslider_register_block_pattern_categories() {
		register_block_pattern_category(
			'gutenslider',
			array( 'label' => __( 'Gutenslider', 'gutenslider' ) )
		);
	}
}
...
if (!function_exists('gutenslider_register_block_patterns')) {
	function gutenslider_register_block_patterns() {
		register_block_pattern(
			'gutenslider/pattern-testimonial-slider',
			array(
				'title'       => __( 'Testimonial Slider', 'gutenslider' ),
				'categories'  => array('gutenslider'),
				'keywords'    => array('slider','rating','quote','testimonial'),
